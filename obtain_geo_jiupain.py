...
def obtain(ak, service_id, entity_name):
    ...
    for hit in res['hits']['hits']:
        ...
        print("orgin = %s %s %s %s %s" % (
        hit["_source"]["open_id"], created_time, hit["_source"]["geo_longitude"], hit["_source"]["geo_latitude"],
        geo_speed))
	bd_lon,bd_lat = bd_encrypt( hit["_source"]["geo_longitude"],hit["_source"]["geo_latitude"] )
        print("jiupian = %s %s %s %s %s" % (
        hit["_source"]["open_id"], created_time, bd_lon,bd_lat,geo_speed))
        url = 'http://yingyan.baidu.com/api/v3/track/addpoint'
        headers = {'Content-type': 'application/x-www-form-urlencoded', 'Cache-Control': 'no-cache'}
        data = {"ak": ak, "service_id": service_id, "entity_name": entity_name,
                "latitude": hit["_source"]["geo_latitude"], "longitude": hit["_source"]["geo_longitude"],
...
if __name__ == '__main__':
    ...
    service_id = 149272
    fileHandle = open('/root/python/employee')
