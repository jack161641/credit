...
and not exists (select '1' from CPT_CST_REQUEST_LOG@ZHENGXIN cz where cz.created_at>=to_date(to_char(sysdate-1,'yyyymmdd')||'00:00:00','yyyymmdd hh24:mi:ss')
and cz.created_at<to_date(to_char(sysdate,'yyyymmdd')||'00:00:00','yyyymmdd hh24:mi:ss')
and g.trade_id=cz.response_sn)""")
rows = cursor.fetchall()
for row in rows:
        day_date = row[0].strftime('%m-%d')
	json_body=[{"measurement":"count_day_ds","tags":{"ds_day":day_date},"fields":{"day":day_date,"ds_name":"上海到上海","count":row[1]}}]
