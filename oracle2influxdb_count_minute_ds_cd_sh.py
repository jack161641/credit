...
and exists (select '1' from CPT_CST_REQUEST_LOG@ZHENGXIN cz where cz.created_at>sysdate-1/24/10
and g.trade_id=cz.response_sn)
) sh_cd_log
group by trans_time""")
rows = cursor.fetchall()
for row in rows:
	json_body=[{"measurement":"count_minute_ds","tags":{"ds_name":"ds_cd_sh"},"fields":{"count":row[1]}}]
